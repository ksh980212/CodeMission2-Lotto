...
public class LottoPrizeDto {
  ...
  private void filterBonusMatched(LottoPrize prize) {
    if (prize.isBonusMatched()) {
      printBonusLottoStatus(prize);
      return;
    }
    printLottoStatus(prize);
  }

  private void printBonusLottoStatus(LottoPrize prize) {
    System.out.println(String.format("%d correct  + bonus Matched (%d amount) %d count",
            prize.getMatchedCount(), prize.getPrizeAmount(),
            lottoPrizeList.stream()
                    .filter(x -> x.getMatchedCount() == prize.getMatchedCount())
                    .filter(LottoPrize::isBonusMatched)
                    .count()));
  }

  private void printLottoStatus(LottoPrize prize) {
    System.out.println(String.format("%d correct (%d amount) %d count",
            prize.getMatchedCount(), prize.getPrizeAmount(),
            lottoPrizeList.stream()
                    .filter(x -> x.getMatchedCount() == prize.getMatchedCount())
                    .filter(x-> !x.isBonusMatched())
                    .count()));
  }

  private long filterYield(LottoPrize prize) {
    if (prize.isDisplayed()) {
      return lottoPrizeList.stream()
              .filter(x -> x.getMatchedCount() == prize.getMatchedCount())
              .filter(x -> x.isBonusMatched() == prize.isBonusMatched())
              .count() * prize.getPrizeAmount();
    }
    return 0;
  }
}
